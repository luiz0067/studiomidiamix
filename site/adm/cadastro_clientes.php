...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,nome,link FROM clientes where (codigo=0".$_GET["codigo"].")";
			...
			else
			{
				...
				echo "<a href=\"../imagem/".$imagem."\">".$imagem."</a><br>";
					if ($_POST ['acao']=='alterar'){
						...
						$sql = "update clientes set imagem='".$imagem."', nome='".$_POST["nome"]."',link='".$_POST["link"]."' where (codigo=".$_POST["codigo"].");";
						echo $sql;
						mysql_query($sql, $link);
					}
					else if( $_POST['acao']=='inserir'){
						$sql = "insert into clientes (imagem, nome, link) values ('".$imagem."','".$_POST["nome"]."','".$_POST["link"]."');";
						//echo $sql;
						mysql_query($sql, $link);
					}
			}
...
?>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem,nome,link FROM clientes;';
			$result = mysql_query($sql, $link);
			if (!$result) {
		...
		?>
				<tr>
					...
					<td>
						...
						<img width="100px" height="50px" src="../imagem/<?php echo $row['imagem'];?>">
					</td>
