...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,comentario FROM maquete where (codigo=0".$_GET["codigo"].")";
...
if ($_POST) {
			...
			else
			{
				...
				echo "<a href=\".\\imagens\\".$imagem."\">".$imagem."</a><br>";
					if ($_POST ['acao']=='alterar'){
						...
						$sql = "update maquete set imagem='".$imagem."', comentario='".$_POST["comentario"]."' where (codigo=".$_POST["codigo"].");";
						echo $sql;
						mysql_query($sql, $link);
					}
					else if( $_POST['acao']=='inserir'){
						$sql = "insert into maquete (imagem, comentario) values ('".$imagem."','".$_POST["comentario"]."');";
						//echo $sql;
						mysql_query($sql, $link);
					}
	...
	if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM maquete where codigo='.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
<center>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem,comentario FROM maquete;';
			$result = mysql_query($sql, $link);
			if (!$result) {
