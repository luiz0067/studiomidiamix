...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,comentario FROM designgrafico where (codigo=0".$_GET["codigo"].")";
...
<?php
if ($_POST) {
	$folder = "C://xampp//htdocs//adm//imagens//";
	if (
		(
	...
	)
	{
		if (($_FILES["imagem"]["size"] < 1024*1024)){
			...
			else
			{
				...
				echo "<a href=\".\\imagens\\".$imagem."\">".$imagem."</a><br>";
					if ($_POST ['acao']=='alterar'){
						//delete_file($folder . $imagem_antiga);
						$sql = "update designgrafico set imagem='".$imagem."', comentario='".$_POST["comentario"]."' where (codigo=".$_POST["codigo"].");";
						echo $sql;
						mysql_query($sql, $link);
					}
					else if( $_POST['acao']=='inserir'){
						$sql = "insert into designgrafico (imagem, comentario) values ('".$imagem."','".$_POST["comentario"]."');";
						//echo $sql;
						mysql_query($sql, $link);
					}
	...
	if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM designgrafico where codigo='.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem,comentario FROM designgrafico;';
			$result = mysql_query($sql, $link);
			if (!$result) {
		...
		?>
				<tr>
					...
					<td>
						<a href="./imagens/<?php echo $row['imagem'];?>"><?php echo $row['imagem'];?><a/><br>
						<img width="160px" height="109px" src="./imagens/<?php echo $row['imagem'];?>">
					</td>
