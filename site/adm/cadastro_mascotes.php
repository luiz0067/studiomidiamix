...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,comentario FROM mascotes where (codigo=0".$_GET["codigo"].")";
...
if ($_POST) {
			...
			else
			{
				...
				echo "<a href=\".\\imagens\\".$imagem."\">".$imagem."</a><br>";
					if ($_POST ['acao']=='alterar'){
						...
						$sql = "update mascotes set imagem='".$imagem."', comentario='".$_POST["comentario"]."' where (codigo=".$_POST["codigo"].");";
						echo $sql;
						mysql_query($sql, $link);
					}
					else if( $_POST['acao']=='inserir'){
						$sql = "insert into mascotes (imagem, comentario) values ('".$imagem."','".$_POST["comentario"]."');";
						//echo $sql;
						mysql_query($sql, $link);
					}
	...
	if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM mascotes where codigo='.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem,comentario FROM mascotes;';
			$result = mysql_query($sql, $link);
			if (!$result) {
