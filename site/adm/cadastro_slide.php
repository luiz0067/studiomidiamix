...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem FROM slide where (codigo=0".$_GET["codigo"].")";
...
<?php
if ($_POST) {
	$folder = "..//slide//demo//dummy_images//";
	if (
		(
	...
	)
	{
		if (($_FILES["imagem"]["size"] < 2000*2000)){
			...
			else
			{
				...
				echo "<a href=\"../slide/demo/dummy_images/".$imagem."\">".$imagem."</a><br>";
...
?>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem FROM slide;';
			$result = mysql_query($sql, $link);
			if (!$result) {
		...
		?>
				<tr>
					...
					<td>
						<a href="../slide/demo/dummy_images/<?php echo $row['imagem'];?>"><?php echo $row['imagem'];?><a/><br>
						<img width="250px" height="150px" src="../slide/demo/dummy_images/<?php echo $row['imagem'];?>">
					</td>
				</tr>
			<?php
				}
