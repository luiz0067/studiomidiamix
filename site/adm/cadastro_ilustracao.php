...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,comentario FROM ilustracao where (codigo=0".$_GET["codigo"].")";
...
if ($_POST) {
	...
	)
	{
		if (($_FILES["imagem"]["size"] < 2000*2000)){
			...
			else
			{
				...
				echo "Size: " . ($_FILES["imagem"]["size"] / 2000) . " Kb<br />";
				...
				echo "<a href=\"../imagem/".$imagem."\">".$imagem."</a><br>";
					if ($_POST ['acao']=='alterar'){
						...
						$sql = "update ilustracao set imagem='".$imagem."', comentario='".$_POST["comentario"]."' where (codigo=".$_POST["codigo"].");";
						echo $sql;
						mysql_query($sql, $link);
					}
					else if( $_POST['acao']=='inserir'){
						$sql = "insert into ilustracao (imagem, comentario) values ('".$imagem."','".$_POST["comentario"]."');";
						//echo $sql;
						mysql_query($sql, $link);
					}
	...
	if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM ilustracao where codigo='.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem,comentario FROM ilustracao;';
			$result = mysql_query($sql, $link);
			if (!$result) {
