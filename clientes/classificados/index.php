...
if (
	...
	if(file_exists("cpf/$cpf.txt")){                         //Se o CPF ja existir não cadastra
		echo "<script>location.href='msg.php?msg=existe_cpf&volt=cadastro.php'</script>";
	}
		...
		if($id==""){
			...
			if(isset($cpf)){
				...
				\$cad=\"$cad\";
				?>";
				$abreid= fopen("cpf/".$cpf.".txt","w");  //cadastrando o cpf
				fwrite($abreid,"$salvar");
				fclose($abreid);
			}
			if(isset($recebemensagem2)){
				...
				\$mensagem2=\"$recebemensagem2\";
				?>";
				fclose ($recebemensagem2=="");
				$abreid= fopen("mensagem2/".$recebemensagem2.".txt","w");  //cadastrando o cpf
				fwrite($abreid,"$salvar");
				fclose($abreid);
			}
			$let="
...
else if ((contarArquivos("usuarios/"))+(contarArquivos("mensagem2/"))<=1002){
?>
			  ...
			  <table width="400" border="0" align="center" cellpadding="0" cellspacing="2">
				...
				<tr>
				  ...
				  <br> <br><br>
					...
					<?php echo (contarArquivos("usuarios/"))+(contarArquivos("mensagem2/"))?> mensagem cadastradas</a>
...
?>
 <?php @include("promocao_encerrada.php"); ?>
<?php } ?>
